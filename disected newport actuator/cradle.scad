// clamped "cradle" for restraining stepper wires in our disected
// newport ppd225 linear actuator
...
difference(){
    union(){
        translate([0,0,length/2-12])
        clamp_cylinder(12, 2.25);
        translate([0,0,-length/2])
        //rotate([0,0,90])
        // rotate_extrude(angle=180)
        // square([d_stepper/2+thickness,15]);
        clamp_cylinder(15, 2.25);
        structure_cylinder();
    }
    // wire through hole
    //cube([2*d_wire, 2*d_wire, length+1], center=true);
    cylinder(h=length+1, r=d_wire, center=true);
    // for (dx=[1,2,3,4]){
    //     translate([-2, d_stepper*(0.5-dx/5),0])
    //     rotate([0,180,0])
    //     cylinder(length+1, r=d_wire/2, center=true);
    // }
